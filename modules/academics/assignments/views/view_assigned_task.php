    ...
    <div class="detail-card">
        ...
        <?php if (!empty($resources)): ?>
            <div class="meta-item">
                ...
                <div style="background: #f8fafc; padding: 1.5rem; border-radius: 12px; border: 2px solid #1a1a1a; display: flex; flex-direction: column; gap: 1.5rem;">
                    ...
                    ?>
                        <div style="padding-bottom: 1rem; border-bottom: 1px dashed var(--border); margin-bottom: 0.5rem;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                                <span style="font-weight: 700;">
                                    <?= getFileIcon($res_path) ?> <?= htmlspecialchars($res_name) ?>
                                </span>
                                <a href="<?= $file_url ?>" download="<?= htmlspecialchars($res_name) ?>" class="neo-pill" style="background: #1a1a1a; color: #fff;">Download</a>
                            </div>
                            ...
                            <div class="preview-container">
                                ...
                                <?php else: ?>
                                    <div style="text-align: center; padding: 1.5rem; background: #fff; border: 2px dashed #cbd5e0; border-radius: 8px; color: #64748b; font-size: 0.85rem;">
        ...
        <?php else: ?>
            <form action="<?= $base_path ?>/api/productivity/submit_assignment" method="POST" enctype="multipart/form-data" id="assignment-submit-form" data-allowed-accept="<?= htmlspecialchars($allowed_accept) ?>" data-allowed-formats="<?= htmlspecialchars($allowed_list_str) ?>" data-max-files="<?= (int)($assignment['max_files'] ?? 1) ?>">
                ...
                <div class="upload-zone">
                    ...
                    <p style="font-size: 0.8rem; color: #1e293b; font-weight: 700; margin-bottom: 0.5rem;">Max files: <?= (int)($assignment['max_files'] ?? 1) ?></p>
                    <?php if (!empty($allowed_list_str)): ?>
                        <p style="font-size: 0.8rem; color: #ef4444; font-weight: 700;">Allowed formats: <?= htmlspecialchars($allowed_list_str) ?></p>
                    <?php else: ?>
